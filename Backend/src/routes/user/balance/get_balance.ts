...
export function getBalance(pool: MyPool): Router {

	const router = express.Router();

	router.get(["/balance", "/balance/:child_id"], expressAsyncHandler(async (req, res) => {
		...
		const active_balace = await User.get_balance_by_id(pool, result_child_id);
		if (!active_balace)
			res.status(200).json({ balance: {} });
		else
			res.status(200).json({ balance: active_balace });
	}));
